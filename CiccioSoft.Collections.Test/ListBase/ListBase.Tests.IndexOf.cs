...
namespace CiccioSoft.Collections.Tests.ListBase
{
    ...
    public abstract partial class ListBase_Tests<T> : IList_Generic_Tests<T>
    {
        ...
        public delegate int IndexOfDelegate(ListBase<T> list, T value);
        ...
        private IndexOfDelegate IndexOfDelegateFromType(IndexOfMethod methodType)
        {
            switch (methodType)
            {
                case (IndexOfMethod.IndexOf_T):
                    return ((ListBase<T> list, T value) => { return list.IndexOf(value); });
                default:
        ...
        {
            _ = frontToBackOrder;
            ListBase<T> list = GenericListFactory(count);
