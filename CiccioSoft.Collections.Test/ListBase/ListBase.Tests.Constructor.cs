...
namespace CiccioSoft.Collections.Tests.ListBase
{
    ...
    public abstract partial class ListBase_Tests<T> : IList_Generic_Tests<T>
    {
        ...
        {
            ListBase<T> list = new ListBase<T>();
        ...
        {
            ListBase<T> list = new ListBase<T>(capacity);
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => new ListBase<T>(capacity));
        }
        ...
        public void Constructor_IEnumerable(EnumerableType enumerableType, int listLength, int enumerableLength, int numberOfMatchingElements, int numberOfDuplicateElements)
        {
            ...
            ListBase<T> list = new ListBase<T>(enumerable);
        ...
        {
            Assert.Throws<ArgumentNullException>(() => { ListBase<T> _list = new ListBase<T>(null); }); //"Expected ArgumentnUllException for null items"
        }
    }
}
