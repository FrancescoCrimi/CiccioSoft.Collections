...
namespace CiccioSoft.Collections.Tests.CollectionBase
{
    ...
    public abstract partial class CollectionBase_Tests<T> : IList_Generic_Tests<T>
    {
        ...
        {
            CollectionBase<T> list = new CollectionBase<T>();
        ...
        {
            CollectionBase<T> list = new CollectionBase<T>(capacity);
        ...
        {
            Assert.Throws<ArgumentNullException>(() => { CollectionBase<T> _list = new CollectionBase<T>(null); }); //"Expected ArgumentnUllException for null items"
        }
    }
}
