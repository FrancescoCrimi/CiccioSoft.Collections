...
namespace CiccioSoft.Collections.Tests.SetBase
{
    public abstract partial class SetBase_Test<T> : ISet_Generic_Tests<T>
    {
        ...
        public void HashSet_Generic_Constructor_IEnumerable(EnumerableType enumerableType, int setLength, int enumerableLength, int numberOfMatchingElements, int numberOfDuplicateElements)
        {
            ...
            SetBase<T> set = new SetBase<T>(enumerable);
        ...
        public void HashSet_Generic_Constructor_IEnumerable_WithManyDuplicates(int count)
        {
            ...
            SetBase<T> hashSetFromDuplicates = new SetBase<T>(Enumerable.Range(0, 40).SelectMany(i => items).ToArray());
            SetBase<T> hashSetFromNoDuplicates = new SetBase<T>(items);
        ...
        public void HashSet_Generic_Constructor_HashSet_SparselyFilled(int count)
        {
            ...
            SetBase<T> set = new SetBase<T>(source, GetIEqualityComparer());
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new SetBase<T>((IEnumerable<T>)null));
            Assert.Throws<ArgumentNullException>(() => new SetBase<T>((IEnumerable<T>)null, EqualityComparer<T>.Default));
        }
        ...
        public void HashSet_Generic_Constructor_IEnumerable_IEqualityComparer(EnumerableType enumerableType, int setLength, int enumerableLength, int numberOfMatchingElements, int numberOfDuplicateElements)
        {
            ...
            SetBase<T> set = new SetBase<T>(enumerable, GetIEqualityComparer());
            Assert.True(set.SetEquals(enumerable));
        }
        ...
        {
            SetBase<T> set = new SetBase<T>(capacity);
            Assert.Equal(0, set.Count);
        }
        ...
        {
            AssertExtensions.Throws<ArgumentOutOfRangeException>("capacity", () => new SetBase<T>(-1));
