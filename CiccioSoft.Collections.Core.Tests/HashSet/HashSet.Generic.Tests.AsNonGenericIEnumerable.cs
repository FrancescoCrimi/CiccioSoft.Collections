...
namespace CiccioSoft.Collections.Core.Tests
{
    public class HashSet_IEnumerable_NonGeneric_Tests : IEnumerable_NonGeneric_Tests
    {
        ...
        {
            var set = new GenericHashSet<string>();
            int seed = 12354;
        ...
        protected override IEnumerable<ModifyEnumerable> GetModifyEnumerables(ModifyOperation operations)
        {
            if ((operations & ModifyOperation.Clear) == ModifyOperation.Clear)
            {
                ...
                {
                    GenericHashSet<string> casted = ((GenericHashSet<string>)enumerable);
                    if (casted.Count > 0)
                    {
                        casted.Clear();
                        return true;
        ...
        protected string CreateT(GenericHashSet<string> set, int seed)
        {
